...
class OrderStateMachine(FSM):
    ...
    def _on_execution(self, event: OrderEvent, *args, **kwargs) -> None:
        pass

    def _on_full_execution(self, event: OrderEvent, *args, **kwargs) -> None:
        pass
...
if __name__ == "__main__":
    ...
    print("✓ Initial state: NEW, not live, not terminal")
    ...
    order.process_event(OrderEvent.EXECUTION)
    order.assert_current_state(OrderState.PARTIALLY_FILLED)
    assert order.is_live == True
    print("✓ After EXECUTION: PARTIALLY_FILLED, is_live=True")

    order.process_event(OrderEvent.EXECUTION)
    order.assert_current_state(OrderState.PARTIALLY_FILLED)
    print("✓ After EXECUTION (self-loop): still PARTIALLY_FILLED")

    order.process_event(OrderEvent.FULL_EXECUTION)
    order.assert_current_state(OrderState.FILLED)
    assert order.is_terminal_state() == True
    assert order.is_live == False
    print("✓ After FULL_EXECUTION: FILLED, terminal, is_live=False")
    ...
    order2.process_event(OrderEvent.FULL_EXECUTION)
    order2.assert_current_state(OrderState.FILLED)
    assert order2.is_terminal_state() == True
    print("✓ ACCEPTED -> FILLED via FULL_EXECUTION")
    ...
    print("✓ After CANCEL_ACK: CANCELLED, terminal, is_live=False")
    ...
    order4.process_event(OrderEvent.EXECUTION)
    ...
    print("✓ After CANCEL_ACK: CANCELLED")
    ...
    order5c.process_event(OrderEvent.EXECUTION)
    ...
    print("✓ PARTIALLY_FILLED -> EXPIRED via TTL_ELAPSED")
