...
class OrderStateMachine(FSM):
    ...
    def __init__(self, order_id: Optional[str] = None, dot_file: Optional[str] = None):
    ...
    def _on_execution(self, event: OrderEvent, quantity: float = 0.0, *args, **kwargs) -> None:
        ...
        label = f"qty={quantity}, total={self._filled_quantity}" if quantity else ""
        self.record_transition(self._current_state, event, label)
    ...
    def _init_dot_file(self) -> None:
        """Initialize the dot file with header."""
        self._dot_file = open(self._dot_file_path, 'w')
        self._dot_file.write('digraph OrderFSM {\n')
        self._dot_file.write('    rankdir=TB;\n')
        self._dot_file.write('    node [shape=box, style=rounded];\n')
        self._dot_file.write(f'    // Order: {self._order_id}\n\n')
        # Mark terminal states with double border
        self._dot_file.write('    // Terminal states\n')
        for state in self.TERMINAL_STATES:
            self._dot_file.write(f'    {state.name} [shape=doubleoctagon];\n')
        self._dot_file.write('\n    // Transitions\n')

    def _log_transition_to_dot(self, from_state: OrderState, to_state: OrderState,
                                event: OrderEvent, label_extra: str = "") -> None:
        ...
        if label_extra:
            label += f"\\n{label_extra}"
        edge = f'    {from_state.name} -> {to_state.name} [label="{label}"];\n'
        self._dot_edges.append(edge)
        self._dot_file.write(edge)

    def finalize_dot(self) -> None:
        """Finalize and close the dot file."""
        if self._dot_file:
            self._dot_file.write('}\n')
            self._dot_file.close()
            self._dot_file = None
            print(f"    [DOT] Wrote {self._dot_file_path}")

    def record_transition(self, new_state: OrderState, event: OrderEvent,
                          label_extra: str = "") -> None:
        """Record state transition in history and optionally log to dot file."""
        # Get the previous state before recording
        if self._state_history:
            prev_state = self._state_history[-1][0]
        else:
            prev_state = OrderState.NEW

        # For the first transition, prev_state is NEW
        if not self._state_history and new_state != OrderState.NEW:
            prev_state = OrderState.NEW

        self._state_history.append((new_state, event))

        # Log to dot file if enabled
        if self._dot_file:
            self._log_transition_to_dot(prev_state, new_state, event, label_extra)
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 60)
    print("GRAPHVIZ DOT TESTS - Nontrivial Paths")
    print("=" * 60)

    # DOT TEST 1: Multiple partial fills with cancel attempts -> filled
    print("\nDOT TEST 1: Complex fill path with cancel rejections")
    dot_order1 = OrderStateMachine(order_id="DOT-001", dot_file="order_complex_fill.dot")
    dot_order1.total_quantity = 1000.0
    dot_order1.process_event(OrderEvent.SUBMIT)
    dot_order1.process_event(OrderEvent.CHECKS_PASSED)
    dot_order1.process_event(OrderEvent.BROKER_ACK)
    dot_order1.process_event(OrderEvent.EXECUTION, quantity=100.0)  # 10%
    dot_order1.process_event(OrderEvent.CANCEL_REJ)  # Try to cancel, rejected
    dot_order1.process_event(OrderEvent.EXECUTION, quantity=150.0)  # 25%
    dot_order1.process_event(OrderEvent.EXECUTION, quantity=200.0)  # 45%
    dot_order1.process_event(OrderEvent.CANCEL_REJ)  # Try again, rejected
    dot_order1.process_event(OrderEvent.EXECUTION, quantity=250.0)  # 70%
    dot_order1.process_event(OrderEvent.EXECUTION, quantity=300.0)  # 100%
    dot_order1.process_event(OrderEvent.FULL_EXECUTION, quantity=0.0)  # Final
    assert dot_order1.quantity_filled() == 1000.0
    dot_order1.finalize_dot()
    print("✓ Generated order_complex_fill.dot")

    # DOT TEST 2: Partial fill then cancel success
    print("\nDOT TEST 2: Partial fill then successful cancel")
    dot_order2 = OrderStateMachine(order_id="DOT-002", dot_file="order_partial_cancel.dot")
    dot_order2.total_quantity = 500.0
    dot_order2.process_event(OrderEvent.SUBMIT)
    dot_order2.process_event(OrderEvent.CHECKS_PASSED)
    dot_order2.process_event(OrderEvent.BROKER_ACK)
    dot_order2.process_event(OrderEvent.EXECUTION, quantity=50.0)
    dot_order2.process_event(OrderEvent.EXECUTION, quantity=75.0)
    dot_order2.process_event(OrderEvent.CANCEL_REJ)  # First cancel attempt fails
    dot_order2.process_event(OrderEvent.EXECUTION, quantity=25.0)  # More fills while waiting
    dot_order2.process_event(OrderEvent.CANCEL_ACK)  # Finally cancelled
    assert dot_order2.quantity_filled() == 150.0
    dot_order2.finalize_dot()
    print("✓ Generated order_partial_cancel.dot")

    # DOT TEST 3: Immediate full execution (simple path)
    print("\nDOT TEST 3: Immediate full execution")
    dot_order3 = OrderStateMachine(order_id="DOT-003", dot_file="order_immediate_fill.dot")
    dot_order3.total_quantity = 100.0
    dot_order3.process_event(OrderEvent.SUBMIT)
    dot_order3.process_event(OrderEvent.CHECKS_PASSED)
    dot_order3.process_event(OrderEvent.BROKER_ACK)
    dot_order3.process_event(OrderEvent.FULL_EXECUTION, quantity=100.0)
    assert dot_order3.quantity_filled() == 100.0
    dot_order3.finalize_dot()
    print("✓ Generated order_immediate_fill.dot")

    # DOT TEST 4: Validation failure (early termination)
    print("\nDOT TEST 4: Validation failure path")
    dot_order4 = OrderStateMachine(order_id="DOT-004", dot_file="order_validation_fail.dot")
    dot_order4.process_event(OrderEvent.SUBMIT)
    dot_order4.process_event(OrderEvent.CHECKS_FAILED)
    dot_order4.finalize_dot()
    print("✓ Generated order_validation_fail.dot")

    # DOT TEST 5: TTL expiration after partial fills
    print("\nDOT TEST 5: TTL expiration after partial fills")
    dot_order5 = OrderStateMachine(order_id="DOT-005", dot_file="order_ttl_expired.dot")
    dot_order5.total_quantity = 200.0
    dot_order5.process_event(OrderEvent.SUBMIT)
    dot_order5.process_event(OrderEvent.CHECKS_PASSED)
    dot_order5.process_event(OrderEvent.BROKER_ACK)
    dot_order5.process_event(OrderEvent.EXECUTION, quantity=30.0)
    dot_order5.process_event(OrderEvent.EXECUTION, quantity=40.0)
    dot_order5.process_event(OrderEvent.CANCEL_REJ)  # Cancel attempt
    dot_order5.process_event(OrderEvent.EXECUTION, quantity=20.0)
    dot_order5.process_event(OrderEvent.TTL_ELAPSED)  # Time's up!
    assert dot_order5.quantity_filled() == 90.0
    dot_order5.finalize_dot()
    print("✓ Generated order_ttl_expired.dot")

    # DOT TEST 6: Broker rejection
    print("\nDOT TEST 6: Broker rejection path")
    dot_order6 = OrderStateMachine(order_id="DOT-006", dot_file="order_broker_reject.dot")
    dot_order6.process_event(OrderEvent.SUBMIT)
    dot_order6.process_event(OrderEvent.CHECKS_PASSED)
    dot_order6.process_event(OrderEvent.BROKER_REJECT)
    dot_order6.finalize_dot()
    print("✓ Generated order_broker_reject.dot")

    print("\n" + "=" * 60)
    print("ALL TESTS PASSED!")
    print("=" * 60)
    print("\nGenerated DOT files:")
    print("  - order_complex_fill.dot")
    print("  - order_partial_cancel.dot")
    print("  - order_immediate_fill.dot")
    print("  - order_validation_fail.dot")
    print("  - order_ttl_expired.dot")
    print("  - order_broker_reject.dot")
    print("\nTo visualize, run: dot -Tpng <file>.dot -o <file>.png")
